  ...
  return (
    <section>
      <div className="flex flex-wrap items-center gap-2 mb-4">
        ...
        <span className="text-[#4B5563] text-xs">Focus shows what was requested. Selected shows what BestStrat actually chose.</span>
      </div>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-[#2B3139]">
                {["Token", "Timeframe", "Focus", "Selected", "Strategy", "Total Return", "Max Drawdown", "Created", ""].map(
                  (col) => (
                    <th
                      key={col}
            ...
            <tbody>
              {runs.map((run, i) => (
                ...
                >
                  ...
                  <td className="px-4 py-3">
                    <FocusBadge value={run.inputFocusLabel || run.inputFocus || "Manual"} />
                  </td>
                  <td className="px-4 py-3">
                    <div className="flex flex-col gap-1.5">
                      <FocusBadge value={run.selectedFocusLabel || run.selectedFocus || run.regime} selected />
                      <span className="text-[#848E9C] text-[11px] whitespace-nowrap">Regime: {run.regime}</span>
                    </div>
                  </td>
                  <td className="px-4 py-3 min-w-44">
                    <span className="text-[#C8CDD6] text-sm leading-snug">{run.strategy}</span>
                  </td>
